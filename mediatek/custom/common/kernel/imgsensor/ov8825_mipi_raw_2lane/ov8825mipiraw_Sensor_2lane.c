...
/*---------------------------------OTP CODE START-------------------------------------------------*/
#define OTP_SUPPORT
//extern kal_uint16 OV8825_write_cmos_sensor(kal_uint32 addr, kal_uint32 para);
//extern kal_uint16 OV8825_read_cmos_sensor(kal_uint32 addr);

extern bool otp_update_wb(unsigned char golden_rg, unsigned char golden_bg);
...
signed char otp_read_wb_group(signed char index)
{
	...
	if ((mid&0x7f) != OTP_MID)
	{
		printk("[LONG-CAM] OTP_MID=%x",mid&0x7f);
		return -1;
	}
...
bool otp_apply_wb(unsigned char golden_rg, unsigned char golden_bg)
{
	...
	////////////////////////////////////////////////////////////////////////////
  /*add by MTK for OTP test*/
  printk("OTP golden_rg=0x%x, golden_bg=0x%x\n", golden_rg, golden_bg); 
  printk("OTP rg_ratio=0x%x, bg_ratio=0x%x\n", rg_ratio, bg_ratio); 
  printk("OTP cmp_rg=%f, cmp_bg=%f\n", cmp_rg, cmp_bg);
  ////////////////////////////////////////////////////////////////////////////
...
printk("[LONG-CAM]start wb update\n");
...
printk("[LONG-CAM]wb update finished\n");
			return 1;
		}
	}
...
static void OV8825_Sensor_Init(void)
{
	...
	//OV8825_write_cmos_sensor(0x5000, 0x06);//	; LENC off, BPC on, WPC on
	OV8825_write_cmos_sensor(0x5000, 0x86);//	; LENC on, BPC on, WPC on added  by ljj 20130227
	
	/*-------OTP CODE START----------------------------------*/
#if defined(OTP_SUPPORT)
	OV8825_write_cmos_sensor(0x0100, 0x01);//	
    	mDELAY(50);
	//otp_update_wb(0x4c, 0x4c);
	otp_update_wb(0x53, 0x50);
	otp_update_lenc();
#endif
/*-------OTP CODE END----------------------------------*/

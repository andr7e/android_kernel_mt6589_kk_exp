...
// hack
// #define GN_MTK_AUTO_DETECT_ALSPS
...
static ssize_t apds9930_show_reg(struct device_driver *ddri, char *buf)
{
    ...
    for (i = 0; i < 20; i++) {
        hwmsen_read_byte_sr(apds9930_i2c_client,addr,&dat);
...
static int __init apds9930_init(void)
{
    ...
    /*if(hwmsen_alsps_sensor_add(&apds9930_init_info)) {
        APS_ERR("apds9930_init failed to register driver\n");
       return -ENODEV;
    }*/
#else 
    if(platform_driver_register(&apds9930_psensor_driver)) {
        APS_ERR("apds9930_init failed to register driver\n");
        return -ENODEV;
    }
#endif
